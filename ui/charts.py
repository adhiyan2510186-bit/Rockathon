...
# Plotly defaults shared by every figure here, so the three look like one system.
_FONT = dict(family="ui-sans-serif, -apple-system, 'Segoe UI', system-ui, sans-serif",
...
def _base_layout(height: int, **kwargs) -> dict:
    ...
    layout = dict(
        ...
        font=_FONT,
        hoverlabel=dict(bgcolor=theme.SURFACE, bordercolor=theme.BORDER_STRONG,
...
def _axis(**kwargs) -> dict:
    ...
    axis = dict(
        ...
        tickfont=dict(size=11, color=theme.INK_MUTED),
    )
    axis.update(kwargs)
    return axis
...
def score_composition(ranked: Sequence[ScoredProduct]) -> go.Figure:
    ...
    for criterion, colour in theme.CRITERION_COLOUR.items():
    ...
    fig.update_layout(**_base_layout(
        ...
        barmode="stack",
        bargap=0.42,
        xaxis=_axis(title=dict(text="score contribution", font=dict(size=11)), range=[0, 100]),
        yaxis=_axis(showgrid=False, tickfont=dict(size=12, color=theme.INK)),
    ))
    return fig
...
def price_history(products: Sequence[Product]) -> go.Figure | None:
    ...
    """
    plotted = [p for p in products if len(p.price_history) >= 2][:3]
    if not plotted:
        return None

    fig = go.Figure()
    for product, colour in zip(plotted, theme.PRODUCT_SERIES):
        fig.add_scatter(
        ...
        fig.add_annotation(
            ...
            xanchor="left", font=dict(size=11, color=theme.INK_SECONDARY),
        )

    fig.update_layout(**_base_layout(
        height=250,
        xaxis=_axis(showgrid=False),
        yaxis=_axis(title=dict(text="₹ per unit", font=dict(size=11)), tickformat=".2f"),
        hovermode="x unified",
...
def stock_burndown(products: Sequence[Product], quantity: int) -> go.Figure | None:
    ...
    """
    plotted = [p for p in products if len(p.stock_history) >= 2][:3]
    if not plotted:
        return None
    ...
    fig.add_hline(
        y=quantity, line=dict(color=theme.INK_MUTED, width=1.5, dash="dot"),
        annotation=dict(text=f"you need {quantity:,}", font=dict(size=11, color=theme.INK_SECONDARY)),
    ...
    fig.update_layout(**_base_layout(
        height=250,
        xaxis=_axis(showgrid=False),
        yaxis=_axis(title=dict(text="units in stock", font=dict(size=11)), tickformat=",.0f"),
        hovermode="x unified",
