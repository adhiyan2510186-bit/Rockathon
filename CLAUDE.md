...
- **Config:** a single `config.yaml` holding the authorisation limit, per-unit
  cap defaults, category default weights, the 5-point substitution threshold,
  and the market-signal thresholds. Limits are read from here, never hardcoded
  in logic and never in a prompt.
